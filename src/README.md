COM 519 ASSESSMENT: Workout Tracker


## Description

 Com 519 assessment tasked the student with building out the MVC parameters for a workout logger that takes in new exercises and uses MongoDB with Mongoose to log them to the database which can then be analyzed in the app's dashboard that has multiple graphic views of the different workouts.
...
Once everything has been set up, the application can be launched by running the command node app.js or npm start. You will then need to visit the local host URL for the port that you have set up for this application. Once loaded in the browser, you can either click the dashboard page to view the
